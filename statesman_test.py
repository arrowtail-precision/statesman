...
class TestMachine(statesman.StateMachine, extra="allow"):
    ...


class TestBaseModel:
    @pytest.fixture()
    def model(self) -> statesman.Base:
        return statesman.Base()
    ...
    def test_add_action(self, model: statesman.Base) -> None:
        assert model._actions == []
        action = statesman.Action(
            callable=lambda: 1234, type=statesman.Action.Types.entry
        )
        model._add_action(action)
        assert model._actions == [action]

    def test_remove_action(self, model: statesman.Base) -> None:
        assert model._actions == []
        action = statesman.Action(
            callable=lambda: 1234, type=statesman.Action.Types.entry
        )
    ...
    class TestCollection:
        @pytest.fixture()
        def model(self, actions: List[statesman.Action]) -> statesman.Base:
            model = statesman.Base()
            model._actions = actions.copy()
            return model

        def test_by_object(
            self, model: statesman.Base, actions: List[statesman.Action]
        ) -> None:
...
class TestState:
    ...
    def test_add_action_invalid_type(self, state: statesman.State) -> None:
        with pytest.raises(
            ValueError,
            match='cannot add state action with type "after": must be "entry" or "exit"',
        ):
...
class TestAction:
    ...
    def test_signature_is_hydrated(self) -> None:
        ...
        assert (
            repr(action.signature)
            == "<Signature (count: int, labels: Dict[str, str]) -> float>"
        )
    ...
    async def test_argument_matching(self) -> None:
        ...
        def action_body(
            count: int, another: bool = False, *args, something=None, number=1234
        ) -> None:
            ...
...
class TestStateMachine:
    @pytest.fixture()
    def state_machine(self) -> TestMachine:
        return TestMachine(states=statesman.State.from_enum(States))

    async def test_get_states_names(self, state_machine: TestMachine) -> None:
        states = state_machine.get_states("starting", "stopped")
        assert len(states) == 2
        assert list(map(lambda i: i.name, states)) == ["starting", "stopped"]

    async def test_get_states_by_state_enum(self, state_machine: TestMachine) -> None:
        states = state_machine.get_states(States)
        assert len(states) == 4
        assert list(map(lambda i: i.name, states)) == [
            "starting",
            "running",
            "stopping",
            "stopped",
    ...
    async def test_get_states_by_state_enum_list(
        self, state_machine: TestMachine
    ) -> None:
        states = state_machine.get_states(States.starting, States.running)
        assert len(states) == 2
        assert list(map(lambda i: i.name, states)) == ["starting", "running"]

    def test_repr(self, state_machine: TestMachine) -> None:
        assert (
            repr(state_machine)
            == "TestMachine(states=[State(name='starting', description='Starting'), State(name='running', description='Running'), State(name='stopping', description='Stopping'), State(name='stopped', description='Stopped')], events=[], state=None)"
        )
...
class TestTransition:
    ...
    def transition(self) -> statesman.Transition:
        ...
        return statesman.Transition(
            state_machine=state_machine, source=starting, target=stopping
        )
    ...
    async def test_params_passed_to_actions(
        self, transition: statesman.Transition
    ) -> None:
        called = False
    ...
    async def test_results_is_none_when_event_is_none(
        self, transition: statesman.Transition
    ) -> None:
    ...
    async def test_results_is_populated_with_return_value_of_on_event_handlers(
        self, mocker
    ) -> None:
        state_machine = TestMachine()
        state_machine.add_states(statesman.State.from_enum(States))
...
class TestProgrammaticStateMachine:
    ...
    def test_enter_states_via_initializer(self) -> None:
        state_machine = TestMachine(states=statesman.State.from_enum(States))
        assert len(state_machine.states) == 4
        state = state_machine.states[0]
        assert state == States.starting
    ...
    async def test_enter_state_name_not_found(self) -> None:
        ...
        with pytest.raises(
            LookupError,
            match='state entry failed: no state was found with the name "invalid"',
        ):
    ...
    async def test_enter_state_not_in_machine(self) -> None:
        ...
        with pytest.raises(
            ValueError,
            match="state entry failed: the state object given is not in the state machine",
        ):
    ...
    async def test_create_no_state(self) -> None:
        state_machine = await TestMachine.create(
            states=statesman.State.from_enum(States)
        )
        assert state_machine.state is None
    ...
    async def test_enter_state_with_args(self, callback, mocker) -> None:
        state_machine = TestMachine(
            states=statesman.State.from_enum(States), state=States.starting
        )
        ...
        assert isinstance(
            callback_mock.call_args.args[0], statesman.Transition
        ), "expected a Transition"
        assert callback_mock.call_args.args[1]
        assert callback_mock.call_args.kwargs == {"foo": "bar"}

    async def test_doesnt_run_state_actions_for_internal_transitions(
        self, mocker
    ) -> None:
        state_machine = TestMachine(
            states=statesman.State.from_enum(States), state=States.starting
        ...
        await state_machine.enter_state(
            States.starting,
            1234,
            foo="bar",
    ...
    async def test_runs_state_actions_for_self_transitions(self, mocker) -> None:
        ...
        on_callback_mock = mocker.spy(state_machine, "on_transition")
        await state_machine.enter_state(
            States.starting, 1234, foo="bar", type_=statesman.Transition.Types.self
        )
        on_callback_mock.assert_called_once()
    ...
    async def test_raises_if_states_and_transition_type_are_inconsistent(
        self,
        target_state: statesman.StateEnum,
        transition_type: statesman.Transition.Types,
        error_message: str,
    ) -> None:
        state_machine = TestMachine(
            states=statesman.State.from_enum(States), state=States.starting
    ...
    class TestEntryConfig:
        async def test_allow(self) -> None:
            state_machine = TestMachine(states=statesman.State.from_enum(States))
            state_machine._state_entry = statesman.Entry.allow
            assert state_machine.state is None
        ...
        async def test_initial(self) -> None:
            ...
            state_machine = TestMachine(states=statesman.State.from_enum(States))
            state_machine._state_entry = statesman.Entry.initial
            assert state_machine.state is None
            assert await state_machine.enter_state(States.starting)
            assert state_machine.state == States.starting
            ...
            with pytest.raises(
                RuntimeError,
                match="state entry failed: `enter_state` is only available to set initial state",
            ):
                assert await state_machine.enter_state(States.stopping)
        ...
        async def test_ignore(self) -> None:
            ...
            state_machine = TestMachine(states=statesman.State.from_enum(States))
            state_machine._state_entry = statesman.Entry.ignore
            assert state_machine.state is None
        ...
        async def test_forbid(self) -> None:
            state_machine = TestMachine(states=statesman.State.from_enum(States))
            state_machine._state_entry = statesman.Entry.forbid
            assert state_machine.state is None
            with pytest.raises(
                RuntimeError,
                match="state entry failed: use of the `enter_state` method is forbidden",
            ):
                assert await state_machine.enter_state(States.starting)
    ...
    def test_add_event_fails_with_unknown_state(self) -> None:
        ...
        with pytest.raises(
            ValueError,
            match='cannot add an event that references unknown states: "invalid"',
        ):
            state_machine.add_event(
                statesman.Event(
                    name="finish",
    ...
    class TestTrigger:
        ...
        async def test_get_event(self, state_machine: TestMachine) -> None:
            event = state_machine.get_event("finish")
            assert event is not None

        async def test_get_event_not_found(self, state_machine: TestMachine) -> None:
            event = state_machine.get_event("invalid")
            assert event is None

        async def test_get_event_invalid_type_raises(
            self, state_machine: TestMachine
        ) -> None:
            assert state_machine.state is None
        ...
        async def test_can_trigger(self, state_machine: TestMachine) -> None:
        ...
        async def test_can_trigger_from_state(self, state_machine: TestMachine) -> None:
            ...
            assert state_machine.can_trigger_event(
                "finish", from_state=state_machine.get_state("starting")
            )

            assert not state_machine.can_trigger_event(
                "reset", from_state=States.starting
            )
            assert not state_machine.can_trigger_event("reset", from_state="starting")
            assert not state_machine.can_trigger_event(
                "reset", from_state=state_machine.get_state("starting")
            )

        async def test_can_trigger_from_state_2(
            self, state_machine: TestMachine
        ) -> None:
            assert state_machine.triggerable_events() == []
            assert state_machine.triggerable_events(from_state=None) == []
            assert state_machine.triggerable_events(from_state="stopping") == [
                state_machine.get_event("reset")
            ]
            assert state_machine.triggerable_events(from_state="starting") == [
                state_machine.get_event("finish")
            ]

        async def test_by_name(self, state_machine: TestMachine) -> None:
            await state_machine.enter_state(States.starting)
            assert state_machine.state == States.starting
            await state_machine.trigger_event("finish")
            assert state_machine.state == States.stopping

        async def test_by_event(self, state_machine: TestMachine) -> None:
            await state_machine.enter_state(States.starting)
            assert state_machine.state == States.starting
            event = state_machine.get_event("finish")
        ...
        async def test_trigger_without_state_raises(
            self, state_machine: TestMachine
        ) -> None:
            assert state_machine.state is None
            with pytest.raises(
        ...
        async def test_trigger_from_incompatible_state(
            self, state_machine: TestMachine
        ) -> None:
        ...
        async def test_with_invalid_name(self, state_machine: TestMachine) -> None:
            await state_machine.enter_state(States.starting)
            with pytest.raises(
                LookupError,
                match='event trigger failed: no event was found with the name "invalid"',
            ):
                await state_machine.trigger_event("invalid")

        async def test_with_invalid_type(self, state_machine: TestMachine) -> None:
            await state_machine.enter_state(States.starting)
            with pytest.raises(
                TypeError,
                match='event trigger failed: cannot trigger an event of type "int": 1234',
            ):
                await state_machine.trigger_event(1234)

        class TestReturnTypes:
            @pytest.fixture()
            def event(self, state_machine: TestMachine) -> statesman.Event:
                event = state_machine.get_event("finish")
            ...
            async def test_return_types_override_on_trigger(
                ...
                result = await state_machine.trigger_event(
                    "finish", return_type=return_type
                )
                assert result == expected_return_value

            async def test_transition_return_type(
                self, state_machine: TestMachine, event
            ) -> None:
                await state_machine.enter_state(States.starting)
                assert state_machine.state == States.starting
                event.return_type = statesman.Transition
        ...
        async def test_with_event_not_in_machine(
            self, state_machine: TestMachine
        ) -> None:
        ...
        async def test_cancel_via_guard_state_machine_method(
            self, state_machine: TestMachine, mocker
        ) -> None:
            await state_machine.enter_state(States.starting)
        ...
        async def test_returning_invalid_value_from_guard_raises_value_error(
            ...
            with pytest.raises(
                ValueError,
                match="invalid return value from guard_transition: must return True, False, or None",
            ):
                await state_machine.trigger_event("finish")
            guard_mock.assert_awaited_once()

        async def test_non_assertion_errors_raise(
            self, state_machine: TestMachine, mocker
        ) -> None:
            await state_machine.enter_state(States.starting)
            guard_mock = mocker.patch.object(state_machine, "guard_transition")
        ...
        async def test_guard_with_silence(
            self, state_machine: TestMachine, mocker
        ) -> None:
            state_machine._guard_with = statesman.Guard.silence
            await state_machine.enter_state(States.starting)
            guard_mock = mocker.patch.object(state_machine, "guard_transition")
            guard_mock.return_value = False
            success = await state_machine.trigger_event("finish")
        ...
        async def test_guard_with_warning(
            self, state_machine: TestMachine, mocker
        ) -> None:
            state_machine._guard_with = statesman.Guard.warning
            await state_machine.enter_state(States.starting)
            guard_mock = mocker.patch.object(state_machine, "guard_transition")
            guard_mock.return_value = False
            with pytest.warns(
                UserWarning,
                match="transition guard failure: guard_transition returned False",
            ):
                await state_machine.trigger_event("finish")

        async def test_guard_with_exception(
            self, state_machine: TestMachine, mocker
        ) -> None:
            state_machine._guard_with = statesman.Guard.exception
            await state_machine.enter_state(States.starting)
            guard_mock = mocker.patch.object(state_machine, "guard_transition")
            guard_mock.return_value = False
            with pytest.raises(
        ...
        class TestActions:
            async def test_guard(self, state_machine: TestMachine, mocker) -> None:
                await state_machine.enter_state(States.starting)
                event = state_machine.get_event("finish")
            ...
            async def test_cancel_via_guard_action_bool(
                self, state_machine: TestMachine, mocker
            ) -> None:
                await state_machine.enter_state(States.starting)
                event = state_machine.get_event("finish")
            ...
            async def test_invalid_return_value_from_guard_raises_value_error(
                ...
                with pytest.raises(
                    ValueError,
                    match="invalid return value from guard action: must return True, False, or None",
                ):
                    await state_machine.trigger_event("finish")
                guard_action.assert_called_once()
            ...
            async def test_guard_actions_run_sequentially(
                self, state_machine: TestMachine, mocker
            ) -> None:
                await state_machine.enter_state(States.starting)
                event = state_machine.get_event("finish")
            ...
            async def test_guard_with_warning(
                ...
                with pytest.warns(
                    UserWarning,
                    match="transition guard failure: guard action returned False",
                ):
                    await state_machine.trigger_event("finish")
            ...
            async def test_before(self, state_machine: TestMachine, mocker) -> None:
                await state_machine.enter_state(States.starting)
                event = state_machine.get_event("finish")
            ...
            async def test_after(self, state_machine: TestMachine, mocker) -> None:
                await state_machine.enter_state(States.starting)
                event = state_machine.get_event("finish")
            ...
            async def test_on(self, state_machine: TestMachine, mocker) -> None:
                await state_machine.enter_state(States.starting)
                event = state_machine.get_event("finish")
            ...
            async def test_inheritable_actions(
                self, state_machine: TestMachine, mocker
            ) -> None:
...
class TestDecoratorStateMachine:
    ...
    def test_set_states_declaratively(self) -> None:
        class TestMachine2(TestMachine):
            __state__: TestDecoratorStateMachine.ProcessStates = (
                TestDecoratorStateMachine.ProcessStates.stopping
            )

        state_machine = TestMachine2()
        assert len(state_machine.states) == 4
    ...
    def test_set_states_embedded_enum(self) -> None:
        ...
        assert state_machine.state is None
        assert len(state_machine.states) == 4
        assert state_machine.states[0] == TestMachine2.States.one
        assert state_machine.state is None

    class ProcessLifecycle(TestMachine):
        class States(statesman.StateEnum):
            starting = "Starting..."
            running = "Running..."
            stopping = "Stopping..."
        ...
        async def run(self, transition: statesman.Transition) -> None:
            ...
            self.logs.append(
                f'Process pid {self.pid} is now running (command="{self.command}")'
            )
    ...
    async def test_states_added(self, state_machine: TestMachine) -> None:
        assert len(state_machine.states) == 4
        assert state_machine.states[0].name == "starting"
        assert state_machine.states[0].description == "Starting..."

    async def test_events_added(self, state_machine: TestMachine) -> None:
    ...
    async def test_trigger_event_through_method_call(
        self, state_machine: TestMachine
    ) -> None:
        assert state_machine.state is None
    ...
    async def test_self_transition(self, state_machine: TestMachine, mocker) -> None:
    ...
    async def test_process_lifecycle(self, state_machine: TestMachine, mocker) -> None:
        ...
        assert state_machine.state == States.starting
        assert (
            len(state_machine.logs) == 1
        ), f"Got unexpected logs: {state_machine.logs}"

        await state_machine.run()
        assert (
            'Process pid 31337 is now running (command="ls -al")' in state_machine.logs
        )
    ...
    async def test_enter_states(self, state_machine: TestMachine) -> None:
        assert state_machine.state is None
        await state_machine.enter_state(States.starting)
        assert state_machine.state
        assert state_machine.state == States.starting
...
class TestInitialState:
    class StateMachine(TestMachine):
        class States(statesman.StateEnum):
            starting = "Starting..."
            running = "Running..."
            stopping = "Stopping..."
    ...
    def test_initial_property(self) -> None:
        ...
        assert (
            TestInitialState.StateMachine.States.__initial__
            == TestInitialState.StateMachine.States.stopped
        )
    ...
    def test_initial_state_can_be_overridden(self) -> None:
        state_machine = TestInitialState.StateMachine(
            state=TestInitialState.StateMachine.States.running
        )
    ...
    def test_cannot_set_multiple_initial_states(self) -> None:
        with pytest.raises(
            ValueError,
            match='cannot declare more than one initial state: "States.one" already declared',
        ):
...
async def reset_config() -> None:
    _state_entry = TestMachine._state_entry
    _guard_with = TestMachine._guard_with
    try:
        yield
    finally:
        TestMachine._state_entry = _state_entry
        TestMachine._guard_with = _guard_with
...
#     """
#     original = obj.model_config.get("extra")
#     obj.__class__.model_config = ConfigDict(extra=extra)
#     try:
#         yield obj
#     finally:
#         obj.__class__.model_config = ConfigDict(extra=original)
...
class TestSequencer:
    class StateMachine(statesman.SequencingMixin, TestMachine):
        class States(statesman.StateEnum):
            starting = "Starting..."
            running = "Running..."
            stopping = "Stopping..."
